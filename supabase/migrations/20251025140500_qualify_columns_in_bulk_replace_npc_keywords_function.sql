-- migration: qualify columns in bulk_replace_npc_keywords rpc
-- purpose: avoid ambiguity between OUT parameters and table columns
...
begin
  ...
  select public.npcs.owner_id, public.npcs.deleted_at
    into v_owner_id, v_deleted_at
  from public.npcs
  where public.npcs.id = p_npc_id
  limit 1;
  ...
  update public.npc_keyword_phrases
   ...
   where public.npc_keyword_phrases.npc_id = p_npc_id
     and public.npc_keyword_phrases.deleted_at is null;

  update public.npc_keywords
     set deleted_at = v_now
   where public.npc_keywords.npc_id = p_npc_id
     and public.npc_keywords.deleted_at is null;
  ...
  return query
    ...
    order by result.sort_index, result.created_at, result.keyword_id;
end;
$$;
